...
- **`hyperdx/list-metrics.ps1` / `list-metrics.sh`** — metric-name discovery + diagnosis. When
  ...
  expected-but-missing name against reality, classifying each as **likely RENAMED** (with the closest
  real names) or **likely TRULY ABSENT**. Reads ClickHouse over HTTP (`CH_URL`/`CH_USER`/`CH_PASSWORD`).
